...
module.exports = {
  creat: async function (req, res, next) {
    try {
  ...
  customerTop: async (req, res, next) => {
    try {
      ...
      const customerTotals = response.map((customer) => {
        const totalPembelian = customer.transaksi.reduce((sum, transaksi) => sum + transaksi.total_pembelian, 0);
        const totalItem = customer.transaksi.reduce((sum, transaksi) => sum + transaksi.detail_transaksi.reduce((detailSum, detail) => detailSum + (detail.id_product - (detail.id_product - 1)) * detail.jumlah, 0), 0);
        return {
          name: customer.nama_customer,
